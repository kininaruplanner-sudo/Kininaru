...
export function DashboardClient({
  ...
  const scoreLabel =
    productivityScore >= 80 ? 'Excellent day' :
    productivityScore >= 60 ? 'Good pace' :
    productivityScore >= 40 ? 'Keep going' : "Let's get started"
  ...
  return (
    <div className="p-5 sm:p-6 lg:p-8 max-w-[1400px] mx-auto space-y-6 lg:space-y-8">
      ...
      {/* Hero row: Productivity Score + Today's Overview */}
      <motion.div
        custom={1}
        variants={fadeUp}
        initial="hidden"
      ...
      >
        ...
        <div className={cn(cardVariants({ padding: 'lg', hover: true }), 'flex flex-col items-center justify-center text-center')}>
          ...
          <p className="text-xs text-muted-foreground mt-1">Tasks · Habits · Focus, blended</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-2">
              ...
              {priorityTasks.length === 0 ? (
                <div className="text-center py-6">
                  ...
                  <p className="text-sm text-muted-foreground">No urgent tasks — great job!</p>
                </div>
              ) : (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-semibold text-foreground">Calendar Preview</h2>
              <Link
                href="/calendar"
                className="flex items-center gap-1 text-xs text-primary hover:underline"
              >
        ...
        <div className="space-y-6">
          ...
          <motion.div
            ...
            className={cardVariants({ padding: 'lg', hover: true })}
          >
            <h2 className="font-semibold text-foreground mb-4">Focus Statistics</h2>
            {focusSessions.length === 0 ? (
              <div className="text-center py-4">
                <Timer className="w-8 h-8 text-muted-foreground mx-auto mb-2 opacity-40" />
                <p className="text-sm text-muted-foreground">No focus sessions this week</p>
                <Link
                  href="/focus"
                  className="text-xs text-primary hover:underline mt-1 inline-block"
                >
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: 'Today', value: `${todayFocusMinutes}m` },
                  { label: 'This week', value: `${Math.round(weekFocusMinutes / 60)}h` },
                  { label: 'Sessions today', value: todaySessionCount },
                  { label: 'Avg. session', value: `${avgSessionLength}m` },
                ].map((s) => (
      ...
      >
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-3 text-xs text-muted-foreground">
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-kin-violet" /> Tasks
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-kin-sage" /> Habits
            </span>
          </div>
        </div>
        ...
        {!weeklyHasActivity ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-muted-foreground">No activity logged yet this week</p>
          </div>
        ) : (
