...
import { parseISO, eachDayOfInterval, subDays, isSameMonth, subMonths } from 'date-fns'
...
function TrendBadge({ change }: { change: number | null }) {
  ...
  if (change === 0) return <span className="text-[10px] text-muted-foreground font-medium">stable</span>
  const up = change > 0
  return (
...
function MoodLine({ data }: { data: { date: string; mood: number }[] }) {
  const [hover, setHover] = useState<number | null>(null)
  if (!data.length) return <p className="text-xs text-muted-foreground text-center py-6">Pas encore de données d'humeur</p>
  const max = 5
            ...
            <div key={wi} className="flex flex-col gap-[3px]">
              {week.map((day, di) => {
                ...
                return (
                  <div
                    key={di}
                    title={`${format(day, 'MMM d')} : ${score} activités`}
                    style={{
                      width: cell,
                      height: cell,
...
export function AnalyticsClient({ tasks, focusSessions, habits, habitLogs, journalEntries }: Props) {
  ...
  const kpis = [
    {
      label: 'Heures de focus', value: Math.round(current.focusMins / 60), change: pctChange(current.focusMins, previous?.focusMins),
      icon: Timer, color: 'text-primary', bg: 'bg-primary/10',
    },
    {
      label: 'Tâches faites', value: current.tasksDone, change: pctChange(current.tasksDone, previous?.tasksDone),
      icon: CheckSquare, color: 'text-kin-sage', bg: 'bg-kin-sage/10',
    },
    {
      label: 'Habitudes cochées', value: current.habitCount, change: pctChange(current.habitCount, previous?.habitCount),
      icon: Repeat2, color: 'text-accent', bg: 'bg-accent/10',
    },
    {
      label: 'Humeur moy.', value: current.avgMood !== null ? `${current.avgMood.toFixed(1)}/5` : '—',
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-4 border-b border-border bg-card">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Aperçu des {range} derniers jours{selectedHabit ? ` · ${selectedHabit.title}` : ''}
          </p>
        </div>
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <motion.div custom={4} variants={fadeUp} initial="hidden" animate="visible" className={cardVariants({ padding: 'md' })}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-semibold text-foreground">Tâches terminées</h3>
                <span className="ml-auto text-xs text-muted-foreground">{range <= 30 ? `${range} derniers jours` : '13 dernières semaines'}</span>
              </div>
            ...
            <motion.div custom={5} variants={fadeUp} initial="hidden" animate="visible" className={cardVariants({ padding: 'md' })}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-semibold text-foreground">Temps de focus</h3>
            ...
            <motion.div custom={6} variants={fadeUp} initial="hidden" animate="visible" className={cardVariants({ padding: 'md' })}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-semibold text-foreground">
                  Habitudes cochées{selectedHabit ? ` — ${selectedHabit.title}` : ''}
                </h3>
              </div>
            ...
            <motion.div custom={7} variants={fadeUp} initial="hidden" animate="visible" className={cardVariants({ padding: 'md' })}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-semibold text-foreground">Tendance d'humeur</h3>
                <span className="ml-auto text-xs text-muted-foreground">Échelle 1–5</span>
              </div>
          ...
          <motion.div custom={8} variants={fadeUp} initial="hidden" animate="visible" className={cardVariants({ padding: 'md' })}>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-semibold text-foreground">
                Carte de chaleur{selectedHabit ? ` — ${selectedHabit.title}` : ''}
              </h3>
              <span className="ml-auto text-xs text-muted-foreground">90 derniers jours</span>
            </div>
            <ActivityHeatmap
              days={allDays}
              getScore={(day) => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <motion.div custom={9} variants={fadeUp} initial="hidden" animate="visible" className={cardVariants({ padding: 'md' })}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="ml-auto text-xs text-muted-foreground">vs semaine dernière</span>
              </div>
              <div className="space-y-3">
                {[
                  { label: 'Focus', curr: `${Math.round(thisWeek.focusMins / 60)}h`, change: pctChange(thisWeek.focusMins, lastWeek?.focusMins) },
                  { label: 'Tâches faites', curr: thisWeek.tasksDone, change: pctChange(thisWeek.tasksDone, lastWeek?.tasksDone) },
                  { label: 'Habitudes cochées', curr: thisWeek.habitCount, change: pctChange(thisWeek.habitCount, lastWeek?.habitCount) },
                  {
                    label: 'Humeur moy.', curr: thisWeek.avgMood !== null ? `${thisWeek.avgMood.toFixed(1)}/5` : '—',
            ...
            <motion.div custom={10} variants={fadeUp} initial="hidden" animate="visible" className={cardVariants({ padding: 'md' })}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-semibold text-foreground">Rapport mensuel</h3>
              ...
              <div className="space-y-3">
                {[
                  ...
                  { label: 'Tâches faites', curr: thisMonth.tasksDone, change: pctChange(thisMonth.tasksDone, lastMonthMetrics?.tasksDone) },
                  { label: 'Habitudes cochées', curr: thisMonth.habitCount, change: pctChange(thisMonth.habitCount, lastMonthMetrics?.habitCount) },
                  {
                    label: 'Humeur moy.', curr: thisMonth.avgMood !== null ? `${thisMonth.avgMood.toFixed(1)}/5` : '—',
          ...
          {habits.length === 0 && tasks.length === 0 && focusSessions.length === 0 && habitLogs.length === 0 && journalEntries.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-foreground font-medium">Pas encore de données</p>
              <p className="text-sm text-muted-foreground mt-1">Utilisez Tâches, Focus ou Habitudes et vos tendances apparaîtront ici</p>
            </div>
          )}
        </div>
      </div>
