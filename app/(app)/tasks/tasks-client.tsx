...
const SORT_OPTIONS: { value: SortKey; label: string }[] = [
  ...
  { value: 'due', label: "Date d'échéance" },
...
const KANBAN_COLUMNS: { status: Status; label: string; color: string }[] = [
  ...
  { status: 'done', label: 'Terminées', color: 'bg-kin-sage/10' },
]
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-4 border-b border-border bg-card">
        <div>
          <h1 className="text-xl font-serif font-bold text-foreground">Tâches</h1>
          <p className="text-xs text-muted-foreground mt-0.5">
            {doneTasks} sur {topLevelTasks.length} terminées &middot; {completionRate}%
          </p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2 px-6 py-3 border-b border-border bg-card/50">
        ...
        {(['all', 'urgent', 'high', 'medium', 'low'] as const).map((f) => (
          ...
          >
            {f === 'all' ? 'Toutes' : PRIORITY_CONFIG[f].label}
          </button>
        ))}
        ...
        <AnimatePresence mode="wait">
          ...
          {view === 'kanban' && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              transition={{ duration: 0.2 }}
              className="flex gap-4 min-h-full overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0"
            >
              {KANBAN_COLUMNS.map((col) => {
                ...
                return (
                  <div
                    key={col.status}
                    className="flex-1 min-w-[220px] sm:min-w-[240px] shrink-0"
                    onDragOver={(e) => {
                      e.preventDefault()
                        ...
                        return (
                          ...
                          >
                            ...
                            {col.status !== 'done' && (
                              ...
                              >
                                Déplacer vers {col.status === 'todo' ? 'En cours' : 'Terminé'}
                              </button>
                            )}
                          </motion.div>
                        )
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <h2 className="text-lg font-serif font-bold text-foreground">Nouvelle tâche</h2>
