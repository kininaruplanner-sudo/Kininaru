  ...
  if (groups.length === 0) {
    return (
      <div className="text-center py-16 text-muted-foreground">
        ...
        <button onClick={onCreateEvent} className="mt-2 text-sm text-primary hover:underline">
      ...
      {groups.map((group, gi) => (
        ...
        >
          <h3 className="text-sm font-semibold text-foreground mb-2 sticky top-0 bg-background/95 backdrop-blur-sm py-1 z-10">
            ...
            {isToday(group.date) && <span className="ml-2 text-primary text-xs font-medium">Aujourd'hui</span>}
          </h3>
    ...
    <div className="flex h-full">
      ...
      <div className="hidden lg:flex w-64 shrink-0 border-r border-border flex-col bg-card/40">
        ...
        <div className="mt-auto p-4 border-t border-border text-xs text-muted-foreground space-y-1.5">
          <p className="font-medium text-foreground mb-2">Raccourcis</p>
          {[
            ['←  →', 'Précédent / suivant'],
            ['T', "Aller à aujourd'hui"],
            ['M S J A', 'Changer de vue'],
            ['N', 'Nouvel événement'],
          ].map(([keys, label]) => (
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-4 border-b border-border bg-card">
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-serif font-bold text-foreground">Calendrier</h1>
          ...
          <AnimatePresence mode="wait">
            ...
            {view === 'agenda' && (
              <motion.div
                ...
                transition={{ duration: 0.2, ease: [0.4, 0, 0.2, 1] }}
              >
                <AgendaView events={events} onDelete={deleteEvent} onCreateEvent={() => setShowModal(true)} />
              </motion.div>
            )}
          </AnimatePresence>
        </div>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <h2 className="text-lg font-serif font-bold text-foreground">Nouvel événement</h2>
