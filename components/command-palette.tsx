...
export function CommandPalette() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      const [tasksRes, eventsRes, habitsRes, journalRes] = await Promise.all([
      ...
      const items: PaletteItem[] = [
        ...
        ...(journalRes.data ?? []).map((j) => ({
          id: `journal-${j.id}`,
          label: j.content ? j.content.slice(0, 60) : j.entry_date,
          hint: 'Journal',
          icon: BookOpen,
          group: 'Résultats' as const,
          ...
          >
            ...
            <div className="flex items-center gap-4 px-5 py-3 border-t border-border text-[11px] text-muted-foreground">
              ...
              <span className="hidden sm:flex items-center gap-1 ml-auto">
                <kbd className="px-1 py-0.5 rounded bg-muted border border-border text-[10px]">?</kbd>
                raccourcis
              </span>
            </div>
          </motion.div>
